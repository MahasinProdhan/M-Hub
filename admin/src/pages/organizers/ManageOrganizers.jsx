import { useEffect, useState } from "react";
import AdminLayout from "../../components/AdminLayout.jsx";
import { apiRequest } from "../../services/api.js";

const ManageOrganizers = () => {
  const [allOrganizers, setAllOrganizers] = useState([]);
  const [organizers, setOrganizers] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");

  const [filters, setFilters] = useState({
    course: "",
    semester: "",
    year: "",
    subject: "",
  });

  const fetchOrganizers = async () => {
    try {
      setLoading(true);
      const res = await apiRequest("/organizers");
      setAllOrganizers(res.data);
      setOrganizers(res.data);
    } catch (err) {
      setError(err.message);
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    fetchOrganizers();
  }, []);

  const applyFilters = () => {
    let filtered = [...allOrganizers];

    if (filters.course) {
      filtered = filtered.filter((o) => o.course === filters.course);
    }

    if (filters.semester) {
      filtered = filtered.filter(
        (o) => o.semester === Number(filters.semester),
      );
    }

    if (filters.year) {
      filtered = filtered.filter((o) => o.year === Number(filters.year));
    }

    if (filters.subject) {
      filtered = filtered.filter((o) =>
        o.subject.toLowerCase().includes(filters.subject.toLowerCase()),
      );
    }

    setOrganizers(filtered);
  };

  const handleDelete = async (id) => {
    const confirmDelete = window.confirm(
      "Are you sure you want to delete this organizer?",
    );

    if (!confirmDelete) return;

    try {
      await apiRequest(`/admin/organizers/${id}`, {
        method: "DELETE",
      });

      setAllOrganizers((prev) => prev.filter((o) => o._id !== id));
      setOrganizers((prev) => prev.filter((o) => o._id !== id));
    } catch (err) {
      alert(err.message);
    }
  };

  return (
    <AdminLayout>
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-2xl font-semibold text-textPrimary">
          Manage Organizers
        </h1>
        <p className="mt-1 text-sm text-textSecondary">
          View and delete organizer collections
        </p>
      </div>

      {/* Filters */}
      <div className="grid grid-cols-1 gap-4 p-4 mb-6 md:grid-cols-4 card shadow-card">
        <select
          value={filters.course}
          onChange={(e) => setFilters({ ...filters, course: e.target.value })}
          className="h-10 px-3 border rounded-md border-borderLight"
        >
          <option value="">All Courses</option>
          <option value="btech">BTech</option>
          <option value="bca">BCA</option>
          <option value="bsc">BSc</option>
          <option value="ba">BA</option>
        </select>

        <select
          value={filters.semester}
          onChange={(e) => setFilters({ ...filters, semester: e.target.value })}
          className="h-10 px-3 border rounded-md border-borderLight"
        >
          <option value="">All Semesters</option>
          {[1, 2, 3, 4, 5, 6, 7, 8].map((s) => (
            <option key={s} value={s}>
              {s}
            </option>
          ))}
        </select>

        <input
          type="number"
          placeholder="Year"
          value={filters.year}
          onChange={(e) => setFilters({ ...filters, year: e.target.value })}
          className="h-10 px-3 border rounded-md border-borderLight"
        />

        <input
          type="text"
          placeholder="Search subject"
          value={filters.subject}
          onChange={(e) => setFilters({ ...filters, subject: e.target.value })}
          className="h-10 px-3 border rounded-md border-borderLight"
        />

        <button
          onClick={applyFilters}
          className="h-10 text-white rounded-md bg-primary md:col-span-1"
        >
          Apply Filters
        </button>
      </div>

      {/* States */}
      {loading && (
        <p className="text-sm text-textSecondary">Loading organizers...</p>
      )}

      {error && (
        <p className="p-3 text-sm rounded-md text-danger bg-red-50">{error}</p>
      )}

      {!loading && organizers.length === 0 && (
        <p className="text-sm text-textSecondary">No organizers found.</p>
      )}

      {/* Table */}
      {!loading && organizers.length > 0 && (
        <div className="p-4 overflow-x-auto card shadow-card">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left border-b text-textSecondary">
                <th className="py-2">Title</th>
                <th>Subject</th>
                <th>Course</th>
                <th>Semester</th>
                <th>Year</th>
                <th>Action</th>
              </tr>
            </thead>

            <tbody>
              {organizers.map((o) => (
                <tr key={o._id} className="border-b last:border-none">
                  <td className="py-2 font-medium text-textPrimary">
                    {o.title}
                  </td>
                  <td>{o.subject}</td>
                  <td>{o.course}</td>
                  <td>{o.semester}</td>
                  <td>{o.year}</td>
                  <td>
                    <button
                      onClick={() => handleDelete(o._id)}
                      className="text-sm font-medium text-danger hover:underline"
                    >
                      Delete
                    </button>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      )}
    </AdminLayout>
  );
};

export default ManageOrganizers;
